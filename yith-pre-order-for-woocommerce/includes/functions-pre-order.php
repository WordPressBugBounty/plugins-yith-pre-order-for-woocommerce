...
if ( ! function_exists( 'ywpo_cart_contains_pre_order_product' ) ) {
	...
	function ywpo_cart_contains_pre_order_product() {
		if ( WC()->cart && did_action( 'wp_loaded' ) && class_exists( 'YITH_Pre_Order_Frontend' ) ) {
			foreach ( WC()->cart->get_cart() as $cart_item ) {
				$product = isset( $cart_item['data'] ) && $cart_item['data'] instanceof WC_Product ? $cart_item['data'] : false;
				if ( $product && YITH_Pre_Order_Utils::is_pre_order_active( $product ) ) {
					return true;
				}
			}
		}
...
if ( ! function_exists( 'ywpo_count_pre_order_items_on_cart' ) ) {
	...
	function ywpo_count_pre_order_items_on_cart() {
		$counter = 0;
		if ( WC()->cart && class_exists( 'YITH_Pre_Order_Frontend' ) ) {
			foreach ( WC()->cart->get_cart() as $cart_item ) {
				$product = isset( $cart_item['data'] ) && $cart_item['data'] instanceof WC_Product ? $cart_item['data'] : false;
				if ( $product && YITH_Pre_Order_Utils::is_pre_order_active( $product ) ) {
